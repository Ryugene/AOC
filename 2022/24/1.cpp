...
void bfs(vector<vector<char>>& grid, map<pair<int,int>,vector<char>>& m, vector<vector<int>>& bfs_grid) {
    ...
    int steps = 1;
    while (true) {
        if (bfs_grid[grid.size()-1][grid[0].size()-2] != 1000)
            return;
...
int main() {
    ...
    while(getline(f,line)) {
        ...
        vector<int> bfs_row(row.size(),1000);
        bfs_grid.push_back(bfs_row);
        grid.push_back(row);
        ++y;
    }
    bfs(grid,m,bfs_grid);
    cout << bfs_grid[grid.size()-1][grid[0].size()-2];
}