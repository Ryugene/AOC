...
void printactive(vector<vertex*> active) {
    for (auto&& v : active) {
        cout << v->index <<endl;
    }
    cout << endl;
...
int main() {
    ...
    active.push_back(vertices[9]);active.push_back(vertices[51]);active.push_back(vertices[35]);active.push_back(vertices[52]);
    active.push_back(vertices[53]);active.push_back(vertices[17]);active.push_back(vertices[37]);
    int res2 = solve(vertices[0],26,active,0,distances,non_zero_vertices);
    cout << res << endl << res2 << endl << res+res2;
}