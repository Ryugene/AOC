    ...
    for (int r = 1; r <= 20; ++r) {
        ...
        for (auto&& m : vm) {
            for (int i = 0; i < m.items.size(); ++i) {
                ...
                m.items[i] = (long double)(m.items[i])/double(3.0);
                if (m.items[i] % m.test_number == 0)
                    vm[m.truethrow].items.push_back(m.items[i]);
                else
                    vm[m.falsethrow].items.push_back(m.items[i]);      
            }
            m.items.clear();
            ++monke;
        }
